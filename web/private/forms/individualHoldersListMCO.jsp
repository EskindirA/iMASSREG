...
<%
    ...
    saveurl = request.getContextPath() + "/Index?action=" + Constants.ACTION_SAVE_HOLDER_MCO;
    viewurl = request.getContextPath() + "/Index?action=" + Constants.ACTION_VIEW_INDIVIDUAL_HOLDER_MCO;
    editurl = request.getContextPath() + "/Index?action=" + Constants.ACTION_EDIT_INDIVIDUAL_HOLDER_MCO;
    deleteurl = request.getContextPath() + "/Index?action=" + Constants.ACTION_DELETE_INDIVIDUAL_HOLDER_MCO;
    backurl = request.getContextPath() + "/Index?action=" + Constants.ACTION_VIEW_PARCEL_MCO;
    if (currentParcel.hasOrphanHolder()) {
        nexturl = request.getContextPath() + "/Index?action=" + Constants.ACTION_GUARDIANS_LIST_MCO;
    } else {
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading"> 
                    ...
                    <span style='float:right' class=''><%=CommonStorage.getText("holders_count")%>:<%=currentParcel.getHolderCount()%></span>
                </div>
                    ...
                    <div class="col-lg-6">
                        <div class="row">
                            <%
                                ...
                                out.println("<button type='submit' id = 'nextButton' name = 'nextButton' class='btn btn-default col-lg-2 col-lg-offset-9' >" + CommonStorage.getText("next") + "</button>");
        ...
        bootbox.confirm("<%=CommonStorage.getText("are_you_sure_you_want_to_delete_this_holder")%> ?", function (result) {
            if (result) {
                $.ajax({
                    type: 'POST',
                    url: "<%=deleteurl%>",
                    data: {"registeredOn": regOn, "holderId": holderId, "upi": '<%=request.getParameter("upi")%>'},
                    error: showajaxerror,
                    success: loadInPlace
                });
            }
