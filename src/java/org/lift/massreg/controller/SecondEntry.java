...
public class SecondEntry {
    ...
    /**
     * Handlers request for getting the add parcel form by the second entry
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void addParcel(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!MasterRepository.getInstance().parcelExists(request.getAttribute("upi").toString(), (byte) 1)) {
          ...
          // already exists
            ...
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_ADD_PARCEL_SEO));
            rd.forward(request, response);
        }
    }

    /**
     * Handlers request for getting the edit parcel form by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    /**
     * Handlers request to delete a parcel by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    ...
    /**
     * Handlers request for viewing a parcel by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void viewParcel(HttpServletRequest request, HttpServletResponse response)
        ...
        // if the parcel does exist in database
        if (request.getAttribute("currentParcel") != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_VIEW_PARCEL_SEO));
            rd.forward(request, response);
        } else {
            request.getSession().setAttribute("title", "Error");
    ...
    /**
     * Handlers request to save parcel form by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void saveParcel(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            p.setStage(CommonStorage.getSEStage());
            ...
            if (p.validateForSave()) {
                ...
                request.getSession().setAttribute("returnAction", Constants.ACTION_ADD_PARCEL_SEO);
                RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
                rd.forward(request, response);
            }
    ...
    /**
     * Handlers request to update parcel details by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void updateParcel(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (newParcel.validateForUpdate()) {
                ...
                request.getSession().setAttribute("returnAction", Constants.ACTION_VIEW_PARCEL_SEO);
                RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
                rd.forward(request, response);
            }
        } catch (Exception ex) {
    ...
    /**
     * Handlers request to add organization holder by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void addOrganizationHolder(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_ADD_ORGANIZATION_HOLDER_SEO));
        rd.forward(request, response);
    }

    /**
     * Handlers request to view list of individual holders by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    /**
     * Handlers request to save a holder by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    ...
    /**
     * Handlers request to view parcel holder by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    ...
    protected static void saveOrganizationHolder(HttpServletRequest request, HttpServletResponse response)
        ...
        if (parcel != null) {
            ...
            try {
                ...
                oh.setStage(CommonStorage.getSEStage());
    ...
    protected static void updateOrganizationHolder(HttpServletRequest request,
        ...
        try {
            ...
            if (newOrganizationHolder.validateForUpdate()) {
                ...
                request.getSession().setAttribute("message", "Sorry, there was a validation error ");
                request.getSession().setAttribute("returnTitle", "Go back to holder list");
                request.getSession().setAttribute("returnAction", Constants.ACTION_VIEW_HOLDER_SEO);
                RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
                rd.forward(request, response);
            }
    ...
    /**
     * Handlers request for viewing an individual holder by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    protected static void viewIndividualHolder(HttpServletRequest request, HttpServletResponse response)
        ...
        // if the parcel does exist in database
        if (request.getAttribute("currentParcel") != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_VIEW_INDIVIDUAL_HOLDER_SEO));
            rd.forward(request, response);
        } else {
            request.getSession().setAttribute("title", "Error");
    ...
    /**
     * Handlers request to update individual holder information by the second
     * entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    /**
     * Handlers request to delete an individual holder by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    /**
     * Handlers request to view organization holder by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void viewOrganizationHolder(HttpServletRequest request, HttpServletResponse response)
        ...
        // if the parcel does exist in database
        if (request.getAttribute("currentParcel") != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_VIEW_ORGANIZATION_HOLDER_SEO));
            rd.forward(request, response);
        } else {
            addOrganizationHolder(request, response);
        }
    ...
    /**
     * Handlers request for getting the edit individual holder form by the
    ...
    protected static void editIndividualHolder(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!MasterRepository.getInstance().parcelExists(request.getAttribute("upi").toString(), (byte) 2)) {
            ...
            request.setAttribute("page", IOC.getPage(Constants.INDEX_EDIT_INDIVIDUAL_HOLDER_SEO));
    ...
    /**
     * Handlers request for viewing a guardian by the first entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void viewGuardian(HttpServletRequest request, HttpServletResponse response)
        ...
        // if the parcel does exist in database
        if (request.getAttribute("currentParcel") != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_VIEW_GUARDIAN_SEO));
            rd.forward(request, response);
        } else {
            request.getSession().setAttribute("title", "Error");
    ...
    /**
     * Handlers request to save a person with interest by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
        ...
        if (parcel != null) {
            ...
            try {
                ...
                if (pwi.validateForSave()) {
                    ...
                    request.getSession().setAttribute("returnAction", Constants.ACTION_PERSONS_WITH_INTEREST_LIST_SEO);
                    RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
                    rd.forward(request, response);
                }
    ...
    /**
     * Handlers request for viewing a person with interest by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    protected static void viewPersonWithInterest(HttpServletRequest request, HttpServletResponse response)
        ...
        // if the parcel does exist in database
        if (request.getAttribute("currentParcel") != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_VIEW_PERSON_WITH_INTEREST_SEO));
            rd.forward(request, response);
        } else {
            request.getSession().setAttribute("title", "Error");
            request.getSession().setAttribute("message", "Sorry, the holder your are looking for dose not exist in the database");
    ...
    /**
     * Handlers request for getting the edit person with form by the second
     * entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    protected static void editPersonWithInterest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (currentParcel == null) {
            ...
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_EDIT_PERSON_WITH_INTEREST_SEO));
            rd.forward(request, response);
        }
    }
    ...
    protected static void editGuardian(HttpServletRequest request, HttpServletResponse response)
        ...
        if (currentParcel == null) {
            ...
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_EDIT_GUARDIAN_SEO));
            rd.forward(request, response);
        }
    }
    ...
    /**
     * Handlers request to view dispute list by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    ...
    /**
     * Handlers request to save a dispute by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void saveDispute(HttpServletRequest request, HttpServletResponse response)
        ...
        if (parcel != null) {
            ...
            try {
                ...
                dispute.setStage(CommonStorage.getSEStage());
                ...
                if (dispute.validateForSave()) {
                    ...
                    request.getSession().setAttribute("message", "Sorry, there was a validation error ");
                    request.getSession().setAttribute("returnTitle", "Go back to dispute list");
                    request.getSession().setAttribute("returnAction", Constants.ACTION_DISPUTE_LIST_SEO);
                    RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
                    rd.forward(request, response);
                }
    ...
    /**
     * Handlers request for viewing a dispute by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void viewDispute(HttpServletRequest request, HttpServletResponse response)
        ...
        // if the parcel does exist in database
        if (request.getAttribute("currentParcel") != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_VIEW_DISPUTE_SEO));
            rd.forward(request, response);
        } else {
            request.getSession().setAttribute("title", "Error");
    ...
    /**
     * Handlers request for getting the edit dispute form by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    protected static void editDispute(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!MasterRepository.getInstance().parcelExists(request.getAttribute("upi").toString(), (byte) 2)) {
            ...
            request.setAttribute("page", IOC.getPage(Constants.INDEX_EDIT_DISPUTE_SEO));
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_EDIT_DISPUTE_SEO));
            rd.forward(request, response);
        }
    }

    /**
     * Handlers request to update dispute information by the second entry
     * operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
    ...
    protected static void updateDispute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            newDispute.setStage(CommonStorage.getSEStage());
    ...
    /**
     * Handlers request to delete a dispute by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    ...
    protected static void updateGuardian(HttpServletRequest request,
        ...
        try {
            ...
            if (newGuardian.validateForUpdate()) {
                ...
                request.getSession().setAttribute("returnAction", Constants.ACTION_GUARDIANS_LIST_SEO);
                RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
                rd.forward(request, response);
            }
   ...
   /**
    ...
    protected static void guardiansList(HttpServletRequest request, HttpServletResponse response)
        ...
        parcel = (Parcel) request.getAttribute("currentParcel");
        if (parcel != null) {
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_GUARDIANS_LIST_SEO));
            rd.forward(request, response);
        } else {
            request.getSession().setAttribute("title", "Error");
            request.getSession().setAttribute("message", "Sorry, the parcel your are looking for dose not exist in the database");
    ...
    /**
     * Handlers request to delete a guardian by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void deleteGuardian(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        if (Guardian.delete(request, Timestamp.valueOf(request.getParameter("registeredOn")), request.getSession().getAttribute("upi").toString(), CommonStorage.getSEStage())) {
            guardiansList(request, response);
        } else {
            // if the parcel fails to validate show error message
            request.getSession().setAttribute("title", "Delete Error");
            ...
            request.getSession().setAttribute("returnTitle", "Go back to guardian list");
            request.getSession().setAttribute("returnAction", Constants.ACTION_GUARDIANS_LIST_FEO);
            RequestDispatcher rd = request.getServletContext().getRequestDispatcher(IOC.getPage(Constants.INDEX_MESSAGE));
            rd.forward(request, response);
        }
    }
    ...
    /**
     * Handlers request to save a guardian by the second entry operator
     *
     * @param request request object passed from the main controller
     * @param response response object passed from the main controller
     */
    protected static void saveGuardian(HttpServletRequest request, HttpServletResponse response)
        ...
        if (parcel != null) {
            ...
            try {
                ...
                guardian.setStage(CommonStorage.getSEStage());
